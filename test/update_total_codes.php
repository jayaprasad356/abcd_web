...
$db = new Database();
$db->connect();
$sql = "SELECT id,total_codes,joined_date FROM `users` WHERE joined_date >= '2023-09-07' AND joined_date < '2023-09-20' AND status = 1 AND code_generate = 1 AND project_type = 'abcd' AND level = 1 ORDER BY joined_date ";
$db->sql($sql);
$res = $db->getResult();
$num = $db->numRows($res);
if ($num >= 1) {
