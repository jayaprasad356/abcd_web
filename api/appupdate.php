...
$appres = $db->getResult();
if($user_id != ''){
    $sql = "SELECT l_referral_count,black_box,code_generate_time,total_referrals,withdrawal,last_updated,device_id,datediff('$date', joined_date) AS history_days,datediff('$datetime', last_updated) AS days,code_generate,withdrawal_status,status,joined_date,today_codes,refer_balance,balance,trial_expired,task_type,champion_task_eligible,trial_count,mcg_timer,security,ongoing_sa_balance,salary_advance_balance,sa_refer_count,refund_wallet,total_refund,earn,refer_code,level,per_code_val,worked_days,project_type,bonus_wallet,earnings_wallet,total_codes,today_mails,total_mails,target_refers,daily_wallet,monthly_wallet,ch_daily_wallet,ch_monthly_wallet,reward_codes  FROM users WHERE id = $user_id ";
    $db->sql($sql);
    $res = $db->getResult();
