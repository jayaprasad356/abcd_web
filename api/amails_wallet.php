...
$amails = (isset($_POST['amails']) && $_POST['amails'] != "") ? $db->escapeString($_POST['amails']) : 0;
...
if($code_generate == 1 && $user_code_generate == 1){
    if($codes != 0){
            ...
            $sql = "INSERT INTO transactions (`user_id`,`amails`,`amount`,`datetime`,`type`,`task_type`)VALUES('$user_id','$amails','$amount','$datetime','$type','$task_type')";
            $db->sql($sql);
            $res = $db->getResult();
        
            $sql = "UPDATE `users` SET  `today_codes` = today_codes + $amails,`total_codes` = total_codes + $amails,`earn` = earn + $amount,`balance` = balance + $amount,`last_updated` = '$datetime' WHERE `id` = $user_id";
            $db->sql($sql);
