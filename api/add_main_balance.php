            ...
            if($level <= 2){
                if($level == 1){
                    if(($total_codes >= 60000) || $worked_days >= $duration){
                        ...
                        $db->sql($sql);
                        $sql = "UPDATE users SET balance= balance + $monthly_wallet,earn = earn + $monthly_wallet,monthly_wallet = 0,old_monthly_wallet = 0,monthly_wallet_status = 0 WHERE id=" . $user_id;
                        $db->sql($sql);
